...
ENV_FILE = "/opt/keycloak-audit/.env"
...
def log_summary(
    *,
    ...
    parts = [
        ...
        f"users_seen={users_seen}", f"users_inserted={users_inserted}",
        f"users_updated={users_updated}", f"users_unchanged={users_unchanged}",
        f"groups_seen={groups_seen}", f"groups_inserted={groups_inserted}",
        f"groups_updated={groups_updated}", f"groups_unchanged={groups_unchanged}",
        f"links_seen={links_seen}", f"links_inserted={links_inserted}",
        f"links_unchanged={links_unchanged}", f"links_removed={links_removed}",
    ]
    if error:
...
def build_snapshot() -> Dict[str, Any]:
    ...
    for user in users:
        ...
        for group in get_user_groups(user_id):
            ...
            if group_id:
                user_group_links.add((user_id, group_id))

    return {
        "users": [
            {
                "id": normalize_text(u.get("id")),
                "username": normalize_text(u.get("username")),
                "firstname": normalize_text(u.get("firstName")),
                "lastname": normalize_text(u.get("lastName")),
            }
            for u in users
            if normalize_text(u.get("id")) and normalize_text(u.get("username"))
        ],
...
def sync_to_db(snapshot: Dict[str, Any]) -> Dict[str, int]:
    stats = {
        ...
        "groups_updated": 0,
        "groups_unchanged": 0,
        "links_seen": len(snapshot["user_groups"]),
        "links_inserted": 0,
        "links_unchanged": 0,
        "links_removed": 0,
    }
    ...
    user_upsert_sql = """
        INSERT INTO public.sso_user (
            id, username, firstname, lastname, active, updated_at
        )
        ...
        SET
            ...
            updated_at = NOW()
        WHERE
            public.sso_user.username IS DISTINCT FROM EXCLUDED.username
            ...
            OR public.sso_user.active IS DISTINCT FROM TRUE
    ...
    group_upsert_sql = """
        ...
        SET
            ...
            updated_at = NOW()
        WHERE
            public.sso_group.name IS DISTINCT FROM EXCLUDED.name
            OR public.sso_group.description IS DISTINCT FROM EXCLUDED.description
            OR public.sso_group.active IS DISTINCT FROM TRUE
        RETURNING (xmax = 0) AS inserted, (xmax <> 0) AS updated
    """
...
def main() -> int:
    try:
        ...
        stats = sync_to_db(snapshot)
        log_summary(status="ok", **stats)
        print(
            "Sync complete: "
            f"users_seen={stats['users_seen']} "
            f"users_inserted={stats['users_inserted']} "
            f"users_updated={stats['users_updated']} "
            f"users_unchanged={stats['users_unchanged']} "
            f"groups_seen={stats['groups_seen']} "
            f"groups_inserted={stats['groups_inserted']} "
            f"groups_updated={stats['groups_updated']} "
            f"groups_unchanged={stats['groups_unchanged']} "
            f"links_seen={stats['links_seen']} "
            f"links_inserted={stats['links_inserted']} "
            f"links_unchanged={stats['links_unchanged']} "
            f"links_removed={stats['links_removed']}"
        )
        return 0
    except requests.RequestException as exc:
        log_summary(
            status="error",
            users_seen=0, users_inserted=0, users_updated=0, users_unchanged=0,
